...
ui <- navbarPage("FlowComb",
             ...
             fluidRow(column(1),
                      ...
                      column(2, h4("Import thresholds from file")), column(3, fileInput("imported_threshold_file",  label =  tags$span(
                        ...
                        tags$i(
                          class = "fa fa-info-circle", 
                          style = "color:#0072B2;font-size: 12px",
                          title = "should contains parameter,number_of_threshold,value,second_value columns\nautomatically generated when a FlowComb analysis has been performed"
                        )), multiple = FALSE, accept = c("thresholds.csv")))),
             
             fluidRow(column(6), 
                      column(3, uiOutput("number_of_threshold_selector"))
                       ),
 ...
 # Analysis panel =========================
    tabPanel("Analysis",
             ...
             fluidRow(column(1),
                      ...
                      column(1, actionButton("PopOI_remove", "remove selected", width = "100%",style = "margin-top: 25px;")),
             ...
             fluidRow(column(3),
                      ...
                      column(6, DTOutput("PopOIs_table"))),
             ...
             fluidRow(column(1),
                      column(2, checkboxInput("stat_on_counts_checkbox", label = "Include analysis on count:", value = FALSE))),
          
             fluidRow(column(3),
                      column(6,  rHandsontableOutput("Scale_factor_table")), 
                      column(2, uiOutput("scale_factors_file_ui"))),
 ...
 tabPanel("Results",
          ...
          fluidRow(column(2, h3("Test Values", 
                          tags$i(
                          class = "fa fa-info-circle", style = "color:#0072B2;font-size: 12px",
                          title = 'All: the frequencies are related tototalFreqcells considered in the analysis\nParent: the frequencies are related to the cell population defined by the combination without the last parameter (e.g. A+B-C+ => A+B-) ' 
                          ))),
                   column(3, uiOutput( "test_values_selector_ui" ) ), column(3,),actionButton("browserbutton", "browser")),
  ...
  # Import data with import data button  ##########
    observeEvent(input$import_data_button, {
      ...
      if(reactV$import_button_status == "totrigger"){
        ...
        reactV$scale_factor_df = data.frame(sample = sub(".csv$", "",perGroup_files_df$name), scale_factor = 1.00) # use for stat on counts
...
# Threshold setting =================================
  ...
  # Output nb of population selector ###########
    ...
    output$number_of_threshold_selector <- renderUI({
      ...
      if(nrow(reactV$thresholds)> 0){
        if(input$Selected_PrmOI_for_threshold %in% reactV$thresholds$parameter){
          prettyRadioButtons( inputId = "number_of_threshold_selector",
                              label =  tags$span(
                                "Number of threshold", 
                                tags$i(
                                  class = "fa fa-info-circle", 
                                  style = "color:#0072B2;font-size: 12px",
                                  title = "For 2 thresholds\n- population correspond to < threshold1\n+ population correspond to >threshold1\n+hi population correspond to > threshold2\n+lo population correspond to < threshold2 and > threshold1\n -&lo population correspond to <threshold2"
                                )), 
                              choices = c("1" = 1, "2" = 2),
                              selected = reactV$thresholds[reactV$thresholds$parameter == input$Selected_PrmOI_for_threshold,"number_of_threshold"],
                              inline = TRUE)
        }else{
          prettyRadioButtons( inputId = "number_of_threshold_selector",
                              label =  tags$span(
  ...
  # Update threshold nb of population #####
    observeEvent(input$number_of_threshold_selector, {
      if(is.null(reactV$thresholds) == FALSE){
        if(nrow(reactV$thresholds)> 0){
          if(input$Selected_PrmOI_for_threshold %in% reactV$thresholds$parameter){
            reactV$thresholds[reactV$thresholds$parameter == input$Selected_PrmOI_for_threshold,"number_of_threshold"] = input$number_of_threshold_selector
            if(input$number_of_threshold_selector == "1" ){
  ...
  # Get threshold from threshold plots click ##################
    reactV$thresholds = data.frame(parameter = character(),  number_of_threshold = numeric(), value = numeric(), second_value = numeric())
    ...
    observeEvent(input$guide_threshold_plot_click, {
      if(input$number_of_threshold_selector == "1"){
        if(input$Selected_PrmOI_for_threshold %in% reactV$thresholds$parameter){
          ...
          new_row = data.frame(parameter = input$Selected_PrmOI_for_threshold, number_of_threshold = 1, value = input$guide_threshold_plot_click$x,second_value = NA  )
          reactV$thresholds = rbind(reactV$thresholds,new_row)
        }        
      }
      
      if(input$number_of_threshold_selector == "2"){
        if(input$Selected_PrmOI_for_threshold %in% reactV$thresholds$parameter){
          ...
          new_row = data.frame(parameter = input$Selected_PrmOI_for_threshold, number_of_threshold = 2, value = input$guide_threshold_plot_click$x,second_value = NA  )
          reactV$thresholds = rbind(reactV$thresholds,new_row)
        }        
      }
    ...
    observeEvent(input$ref_threshold_plot_click, {
      ...
      if(input$number_of_threshold_selector == "1"){
        if(input$Selected_PrmOI_for_threshold %in% reactV$thresholds$parameter){
          ...
          new_row = data.frame(parameter = input$Selected_PrmOI_for_threshold, number_of_threshold = 1, value = input$ref_threshold_plot_click$x,second_value = NA  )
          reactV$thresholds = rbind(reactV$thresholds,new_row)
        }        
      }
      ...
      if(input$number_of_threshold_selector == "2"){
        if(input$Selected_PrmOI_for_threshold %in% reactV$thresholds$parameter){
          ...
          new_row = data.frame(parameter = input$Selected_PrmOI_for_threshold, number_of_threshold = 2, value = input$ref_threshold_plot_click$x,second_value = NA  )
          reactV$thresholds = rbind(reactV$thresholds,new_row)
        }        
      }
    ...
    observeEvent(input$group2_threshold_plot_click, {
      ...
      if(input$number_of_threshold_selector == "1"){
        if(input$Selected_PrmOI_for_threshold %in% reactV$thresholds$parameter){
          ...
          new_row = data.frame(parameter = input$Selected_PrmOI_for_threshold, number_of_threshold = 1, value = input$group2_threshold_plot_click$x,second_value = NA  )
          reactV$thresholds = rbind(reactV$thresholds,new_row)
        }        
      }
      ...
      if(input$number_of_threshold_selector == "2"){
        if(input$Selected_PrmOI_for_threshold %in% reactV$thresholds$parameter){
          ...
          new_row = data.frame(parameter = input$Selected_PrmOI_for_threshold, number_of_threshold = 2, value = input$group2_threshold_plot_click$x,second_value = NA  )
          reactV$thresholds = rbind(reactV$thresholds,new_row)
        }        
      }
    })
...
# Analysis =================================
  ...
  # Population of interest selection #####
    ...
    output$PopOI_1_ui <- renderUI({
      ...
      one_threshold_prm = reactV$thresholds$parameter[reactV$thresholds$number_of_threshold == 1]
      two_thresholds_prm = reactV$thresholds$parameter[reactV$thresholds$number_of_threshold == 2]
      choices_list = as.list(c("None",sort(c(paste0(one_threshold_prm,"-"), paste0(one_threshold_prm,"+"), 
                                             paste0(two_thresholds_prm,"-"), paste0(two_thresholds_prm,"-&lo"), paste0(two_thresholds_prm,"+"), paste0(two_thresholds_prm,"+lo"), paste0(two_thresholds_prm,"+hi") ))))

      choices_list = as.list( unlist(choices_list)[grepl("@",unlist(choices_list))==FALSE])
    ...
    output$PopOI_2_ui <- renderUI({
      ...
      one_threshold_prm = reactV$thresholds$parameter[reactV$thresholds$number_of_threshold == 1]
      one_threshold_prm = one_threshold_prm[one_threshold_prm !=  gsub("[-+]", "", input$PopOI_1) ]
      
      two_thresholds_prm = reactV$thresholds$parameter[reactV$thresholds$number_of_threshold == 2]
      two_thresholds_prm = two_thresholds_prm[two_thresholds_prm !=  gsub("[-+].*", "", input$PopOI_1) ]
    ...
    output$PopOI_3_ui <- renderUI({
      ...
      one_threshold_prm = reactV$thresholds$parameter[reactV$thresholds$number_of_threshold == 1]
      one_threshold_prm = one_threshold_prm[one_threshold_prm !=  gsub("[-+]", "", input$PopOI_1) ]
      one_threshold_prm = one_threshold_prm[one_threshold_prm !=  gsub("[-+]", "", input$PopOI_2) ]
      
      two_thresholds_prm = reactV$thresholds$parameter[reactV$thresholds$number_of_threshold == 2]
      two_thresholds_prm = two_thresholds_prm[two_thresholds_prm !=  gsub("[-+].*", "", input$PopOI_1) ]
      two_thresholds_prm = two_thresholds_prm[two_thresholds_prm !=  gsub("[-+].*", "", input$PopOI_2) ]
    ...
    output$PopOI_4_ui <- renderUI({
      ...
      one_threshold_prm = one_threshold_prm[one_threshold_prm !=  gsub("[-+]", "", input$PopOI_1) ]
      one_threshold_prm = one_threshold_prm[one_threshold_prm !=  gsub("[-+]", "", input$PopOI_2) ]
      one_threshold_prm = one_threshold_prm[one_threshold_prm !=  gsub("[-+]", "", input$PopOI_3) ]
      
      two_thresholds_prm = reactV$thresholds$parameter[reactV$thresholds$number_of_threshold == 2]
      two_thresholds_prm = two_thresholds_prm[two_thresholds_prm !=  gsub("[-+].*", "", input$PopOI_1) ]
      two_thresholds_prm = two_thresholds_prm[two_thresholds_prm !=  gsub("[-+].*", "", input$PopOI_2) ]
      two_thresholds_prm = two_thresholds_prm[two_thresholds_prm !=  gsub("[-+].*", "", input$PopOI_3) ]
    ...
    output$PopOI_5_ui <- renderUI({
      ...
      one_threshold_prm = one_threshold_prm[one_threshold_prm !=  gsub("[-+]", "", input$PopOI_2) ]
      one_threshold_prm = one_threshold_prm[one_threshold_prm !=  gsub("[-+]", "", input$PopOI_3) ]
      one_threshold_prm = one_threshold_prm[one_threshold_prm !=  gsub("[-+]", "", input$PopOI_4) ]
      
      two_thresholds_prm = reactV$thresholds$parameter[reactV$thresholds$number_of_threshold == 2]
      two_thresholds_prm = two_thresholds_prm[two_thresholds_prm !=  gsub("[-+].*", "", input$PopOI_1) ]
      two_thresholds_prm = two_thresholds_prm[two_thresholds_prm !=  gsub("[-+].*", "", input$PopOI_2) ]
      two_thresholds_prm = two_thresholds_prm[two_thresholds_prm !=  gsub("[-+].*", "", input$PopOI_3) ]
      two_thresholds_prm = two_thresholds_prm[two_thresholds_prm !=  gsub("[-+].*", "", input$PopOI_4) ]
    ...
    output$PopOI_6_ui <- renderUI({
      ...
      one_threshold_prm = one_threshold_prm[one_threshold_prm !=  gsub("[-+]", "", input$PopOI_3) ]
      one_threshold_prm = one_threshold_prm[one_threshold_prm !=  gsub("[-+]", "", input$PopOI_4) ]
      one_threshold_prm = one_threshold_prm[one_threshold_prm !=  gsub("[-+]", "", input$PopOI_5) ]
      ...
      two_thresholds_prm = two_thresholds_prm[two_thresholds_prm !=  gsub("[-+].*", "", input$PopOI_3) ]
      two_thresholds_prm = two_thresholds_prm[two_thresholds_prm !=  gsub("[-+].*", "", input$PopOI_4) ]
      two_thresholds_prm = two_thresholds_prm[two_thresholds_prm !=  gsub("[-+].*", "", input$PopOI_5) ]
    ...
    observeEvent(input$PopOI_add, {
      pc = paste(input$PopOI_1,input$PopOI_2,input$PopOI_3,input$PopOI_4,input$PopOI_5,input$PopOI_6, sep = "_" )
      pc = gsub("_None.*", "",pc)
    ...
    observeEvent(input$PopOI_remove, {
      if (!is.null(input$PopOIs_table_rows_selected)) {
        reactV$PopOIs  = reactV$PopOIs[-as.numeric(input$PopOIs_table_rows_selected),]  
      }
    })
    

    output$PopOIs_table = renderDT({
      if(nrow(reactV$PopOIs) == 0)
        return(NULL)
      DT::datatable(reactV$PopOIs, editable = TRUE, rownames = FALSE)})

    
    
    observeEvent(input$PopOIs_table_cell_edit, {
      info = input$PopOIs_table_cell_edit
    ...
    output$Scale_factor_table =  renderRHandsontable({
      if(is.null(reactV$scale_factor_df)|input$stat_on_counts_checkbox == FALSE)
        return(NULL)
      df = reactV$scale_factor_df
      df$scale_factor = format(df$scale_factor,nsmall = 2)
      rhandsontable(df, rowHeaders=FALSE, width = 600, height = 200, readOnly = TRUE, stretchH = "all") %>%
        hot_col("scale_factor", readOnly = FALSE, halign = "htRight") %>%
        hot_cols(columnSorting = TRUE)
    })
    
    
    observeEvent(input$Scale_factor_table$changes$changes, {
      reactV$scale_factor_df = hot_to_r(input$Scale_factor_table)
      reactV$scale_factor_df$scale_factor = format(as.double(reactV$scale_factor_df$scale_factor))
    })
    
    output$scale_factors_file_ui = renderUI({
      if(input$stat_on_counts_checkbox == FALSE)
        return(NULL)
      fileInput("scale_factors_file",  label = tags$span("Load scale_factors.csv file", 
                                                 tags$i(
                                                   class = "fa fa-info-circle", 
                                                   style = "color:#0072B2;font-size: 12px",
                                                   title = "should contains sample and	scale_factor columns"
                                                 ), multiple = FALSE, accept = c(".csv")))
    })
    
    observeEvent(input$scale_factors_file, {
      df = read.csv(input$scale_factors_file$datapath)
      if(all(colnames(df) == c("sample","scale_factor")) == FALSE ){
        report_warning("Scale_factors file incompatibility", 'Column names should be "sample" and "scale_factor"' )
      }else{
        if(all(sort(df$sample) == sort(reactV$scale_factor_df$sample))){
        reactV$scale_factor_df = df
        }else{
          unrecognised_samples = df$sample[!(df$sample %in% reactV$scale_factor_df$sample)]
          report_warning("Scale_factors file incompatibility", paste("Unrecognised sample name:", paste(df$sample[!(df$sample %in% reactV$scale_factor_df$sample)], collapse = ", ")) )
        }
      }
    })
    ...
    observeEvent(input$output_dir_selector,{
      reactV$output_dir = parseDirPath( unique(volumes, input$output_dir_selector) )
    })
    ...
    observeEvent(input$run, {
        ...
        if(all(reactV$PrmOIs %in% reactV$thresholds$parameter) == FALSE ){ ## check that there is a threshold set for each parameter of interest
          report_warning("Threshold(s) missing", paste("Set threshold for:", paste(reactV$PrmOIs[!(reactV$PrmOIs %in% reactV$thresholds$parameter)], collapse = " ")) )
            ...
            if( sum(is.na(reactV$thresholds$second_value[reactV$thresholds$number_of_threshold == 2])) ){
              report_warning("2nd threshold(s) missing", paste("Select 2 populations or set 2nd threshold for:", paste( reactV$thresholds$parameter[reactV$thresholds$number_of_threshold == 2 & is.na(reactV$thresholds$second_value)], collapse = ",")) )
            }else{
          ...
          write.csv(reactV$scale_factor_df, file = paste0("./", outdir, "/", "scale_factors_", time, ".csv") , row.names = FALSE) 

          number_of_thresholds = setNames(reactV$thresholds$number_of_threshold, reactV$thresholds$parameter)
          ...
          L1_pops = c(paste0(names(number_of_thresholds), "-"), paste0(names(number_of_thresholds), "+"))
          if(sum(number_of_thresholds == "2") > 0){L1_pops = c(L1_pops,paste0(names(number_of_thresholds[number_of_thresholds == "2"]), "-&lo"), 
                                                               paste0(names(number_of_thresholds[number_of_thresholds == "2"]), "+lo"), 
                                                               paste0(names(number_of_thresholds[number_of_thresholds == "2"]), "+hi")) }
          
          
          #PopOIs check
          if(nrow(reactV$PopOIs)>0 ){
            for(i in 1:nrow(reactV$PopOIs)){
              PopPOI_prm = strsplit(reactV$PopOIs$parameters_combination[i], "_")[[1]]
              recognised_prm = PopPOI_prm %in% L1_pops
              if(all(recognised_prm) == FALSE){
                report_warning("Unrecognised parameter", paste( paste(PopPOI_prm[recognised_prm == FALSE] , collapse = ","), "not recognised in", reactV$PopOIs$name[i] ))
                remove_modal_spinner()
                return(NULL)
              }
            }
          ...
          if(length(unique(reactV$PopOIs$name)) != length(reactV$PopOIs$name) ){
            pop_name_occ = table(reactV$PopOIs$name)
            report_warning("Population of interest name used several time", paste( paste(names(pop_name_occ)[ pop_name_occ>1] , collapse = ","), "used to define more than one population" ))
            remove_modal_spinner()
            return(NULL)
          }
          
          if( all(grepl("@", reactV$PopOIs$name)) == FALSE ){
            report_warning("Population of interest name with missing @", paste( paste(reactV$PopOIs$name[grepl("@", reactV$PopOIs$name) == FALSE] , collapse = ","), "has to start with @ character" ))
            remove_modal_spinner()
            return(NULL)
          }
          ...
          for(L1_pop in L1_pops){
            ...
            side = gsub(".*[-]","-", gsub(".*[+]", "+",L1_pop))
            if(side == "-"){L1_pops_belonging_matrix[,L1_pop] = as.numeric(aboveThresholdsum_matrix[,parameter] == 0)}
            if(side == "+"){L1_pops_belonging_matrix[,L1_pop] = as.numeric(aboveThresholdsum_matrix[,parameter] %in% c(1,2) )}
            if(side == "-&lo"){L1_pops_belonging_matrix[,L1_pop] = as.numeric(aboveThresholdsum_matrix[,parameter] %in% c(0,1) )}
            if(side == "+lo"){L1_pops_belonging_matrix[,L1_pop] = as.numeric(aboveThresholdsum_matrix[,parameter] == 1 )}
            if(side == "+hi"){L1_pops_belonging_matrix[,L1_pop] = as.numeric(aboveThresholdsum_matrix[,parameter] == 2 )}
          }
          ...
          if(nrow(reactV$PopOIs) > 0){ 
            ...
            reactV$thresholds = rbind(reactV$thresholds, data.frame(parameter = reactV$PopOIs$name, number_of_threshold = 1, value = 500,second_value = NA  ))
          ...
          print(paste0(Sys.time()," Getting totalFreq non repetitive populations ... "))
          update_modal_spinner(text = paste0(Sys.time()," GettingtotalFreqnon repetititive populations ... "))
          ...
          get_parentPop <- function(popv){
            if(length(popv) > 1){
              parent_pop = paste(sort(popv[-length(popv)]),collapse = "_")
              return(parent_pop)
            }else{return("None")}
          } 
          ...
          for(sample in unique(rowData$sample) ){
            perSample_hpopstat$totalFreq[perSample_hpopstat$sample == sample] = perSample_hpopstat$count[perSample_hpopstat$sample == sample]/perSample_totalcount[sample]*100 # expressed as a percentage
          }
          
          perSample_hpopstat$parentFreq = perSample_hpopstat$count / (perSample_hpopstat$parentCount+0.001)*100 # avoid 0 division and expressed as a percentage
          ...
          ## Get normCount ####
          if(input$stat_on_counts_checkbox == TRUE){
            for(i in 1:nrow(reactV$scale_factor_df) ){
              perSample_hpopstat$normCount[perSample_hpopstat$sample == reactV$scale_factor_df$sample[i] ] =  perSample_hpopstat$count[perSample_hpopstat$sample == reactV$scale_factor_df$sample[i] ]*as.numeric(reactV$scale_factor_df$scale_factor[i]) 
            }
            reactV$perSample_hpopstat = perSample_hpopstat
          }
          ...
          if(input$stat_on_counts_checkbox == TRUE){
          reactV$perGroup_hpopstat = perSample_hpopstat %>% 
                                                     ...
                                                     summarise(n=n(), totalFreq_mean = mean(totalFreq), totalFreq_sd = sd(totalFreq), totalFreq_sem = sd(totalFreq)/sqrt(n()),
                                                                  ...
                                                                  normCount_mean = mean(normCount), normCount_sd = sd(normCount), normCount_sem = sd(normCount)/sqrt(n())) %>%
                                                     as.data.frame()
          }else{
            reactV$perGroup_hpopstat = perSample_hpopstat %>% 
                                                       group_by(group, population,non_rep_pop) %>%
          ...
          if(test_type == "t.test"){
            res1 = perSample_hpopstat %>% 
              ...
              mutate(totalFreq_refgroup_mean = mean(unlist(totalFreq_refgroup_values)),
                     ...
                     totalFreq_l2fc = log2( (totalFreq_group2_mean+1e-5) / (totalFreq_refgroup_mean+1e-5)),
                     totalFreq_pvalue = secured_t.test.p.value(unlist(totalFreq_group2_values), unlist(totalFreq_refgroup_values)))  %>%
            ...
            res2 = perSample_hpopstat %>% 
              ...
              mutate(parentFreq_refgroup_mean = mean(unlist(parentFreq_refgroup_values)),
                     ...
                     parentFreq_l2fc = log2( (parentFreq_group2_mean+1e-5) / (parentFreq_refgroup_mean+1e-5)),
                     parentFreq_pvalue = secured_t.test.p.value(unlist(parentFreq_group2_values), unlist(parentFreq_refgroup_values))) %>%
            ...
            test_res$non_rep_pop_logical = test_res$non_rep_pop == test_res$population
              ...
              if(input$stat_on_counts_checkbox == TRUE){
                 res3 = perSample_hpopstat %>% 
                  select(group, population, normCount ) %>%
                  group_by(group, population) %>% 
                  summarise(normCount = list(normCount)) %>% 
                  spread(group, normCount)  %>%
                  rename(normCount_refgroup_values = groups_names[1]) %>%
                  rename(normCount_group2_values = groups_names[2]) %>%
                  group_by(population)  %>%
                  mutate(normCount_refgroup_mean = mean(unlist(normCount_refgroup_values)),
                         normCount_group2_mean = mean(unlist(normCount_group2_values)),
                         normCount_l2fc = log2( (normCount_group2_mean+1e-2) / (normCount_refgroup_mean+1e-2)),
                         normCount_pvalue = secured_t.test.p.value(unlist(normCount_group2_values), unlist(normCount_refgroup_values)))  %>%
                  select(population, normCount_l2fc, normCount_pvalue, normCount_refgroup_mean, normCount_group2_mean)

                test_res= as.data.frame(cbind(test_res, res3[,-1]))
                test_res[as.logical(test_res$min_parentCount_for_test_exclusion_filter+ test_res$min_subpop_count_for_test_exclusion_filter),c("normCount_l2fc","normCount_pvalue")] = NA
                
                test_res$normCount_minuslog10pvalue = -log10(test_res$normCount_pvalue)
                test_res$normCount_distancefrom0 = sqrt(test_res$normCount_l2fc^2 + test_res$normCount_minuslog10pvalue^2)
  ...
  observeEvent(input$loadRDS, {
      ...
      reactV$scale_factor_df = rds_reactV$scale_factor_df
      remove_modal_spinner()
...
# Results =================================
    ...
    output$test_values_selector_ui <- renderUI({
      if( "normCount_pvalue" %in% colnames(reactV$test_res)){
      radioGroupButtons("test_values_selector", label = "", choiceNames =  c("Total frequencies", "Parent frequencies", "Normalised counts" ), choiceValues = c("totalFreq", "parentFreq", "normCount"), individual = TRUE)
      }else{ radioGroupButtons("test_values_selector", label = "", choiceNames =  c("Total frequencies", "Parent frequencies"), choiceValues = c("totalFreq", "parentFreq") , individual = TRUE)}
    })
  ...
  # Generate volcano data table ##########  
    volcano_df <- reactive ({
      if(is.null(reactV$test_res) | is.null(input$test_values_selector))
        return(NULL)

      if(input$test_values_selector == "totalFreq"){
      ...
      if(input$test_values_selector == "parentFreq"){
      ...
      if(input$test_values_selector == "normCount"){
        volcano_df = reactV$test_res
        volcano_df$pvalue = volcano_df$normCount_pvalue
        volcano_df$l2fc = volcano_df$normCount_l2fc
        volcano_df$minuslog10pvalue = volcano_df$normCount_minuslog10pvalue
        volcano_df$distancefrom0 = volcano_df$normCount_distancefrom0
      }
  ...
  # Get last selected HHM population if exist ########
    ...
    volc_label_HHM <- reactive ({
      if(is.null(click$last_Hieracheatmap_population)| is.null(input$test_values_selector))
        return(NULL)
      if(input$label_HHM_checkbox == TRUE){
        if(input$test_values_selector %in% c("totalFreq", "normCount")){
  ...
  # Generate volcano selected population per group population frequency table for barplot table ###############
    barplot_df <-  reactive({
      if(is.null(reactV$perGroup_hpopstat)| is.null(input$test_values_selector))
        return(NULL)
      ...
      if(input$test_values_selector == "normCount"){
        volcpop_perGroup_hpopstat$mean = volcpop_perGroup_hpopstat$normCount_mean
        volcpop_perGroup_hpopstat$sd = volcpop_perGroup_hpopstat$normCount_sd
        volcpop_perGroup_hpopstat$sem = volcpop_perGroup_hpopstat$normCount_sem
        return(volcpop_perGroup_hpopstat)
      }
  ...
  # Output barplot ############### 
    Barplot <- reactive({
      if(is.null(barplot_df()) | is.null(input$test_values_selector))
        return(NULL)
      ...
      barplot_dots_df$group = factor(barplot_dots_df$group, levels = reactV$groups_names )
      if(input$test_values_selector == "totalFreq"){
        barplot_dots_df$value = barplot_dots_df$totalFreq
      }
      if(input$test_values_selector == "parentFreq"){
        barplot_dots_df$value = barplot_dots_df$parentFreq
      }
      if(input$test_values_selector == "normCount"){
        barplot_dots_df$value = barplot_dots_df$normCount
      }

      limit_max = (max(barplot_df$mean)+barplot_df$sem[which.max(barplot_df$mean)])
      if(limit_max < max(barplot_dots_df$value) ) { limit_max = max(barplot_dots_df$value)}

      if(input$test_values_selector == "normCount"){
        plot = ggplot()+
          scale_y_continuous(name = "Population normalised counts (+/- sem)", limits = c(0,limit_max))+
          scale_x_discrete(name = "")+
          scale_fill_manual(values = c("darkgrey","#225EA8"))+
          scale_shape_manual(values = c(21,21))+
          geom_bar(data=barplot_df, stat = "identity", aes(x = population, y = mean, fill = group), position = position_dodge())+
          geom_errorbar(data=barplot_df, aes(x = population, ymin = (mean-sem), ymax =(mean+sem), group = group), width = 0.5, position = position_dodge(width = 0.9) )+
          geom_point(data = barplot_dots_df, aes(x = population, y = value, shape = group ), size = 3,  position = position_dodge(width=0.9))+
  ...
  # Output barplot click population info table ############
    barplot_infotable <- reactive({
      ...
      volcano_df = volcano_df()
      if(input$test_values_selector == "normCount"){
        df = data.frame(population = click$barplot_population, 
                        means = paste0(round(barplot_df$mean[barplot_df$population == click$barplot_population & barplot_df$group == reactV$groups_names[1]],1),
                                       " vs ",
                                       round(barplot_df$mean[barplot_df$population == click$barplot_population & barplot_df$group == reactV$groups_names[-1]],1)),
        ...
        df = data.frame(population = click$barplot_population, 
                   means = paste0(paste0(round(barplot_df$mean[barplot_df$population == click$barplot_population & barplot_df$group == reactV$groups_names[1]],1),"%"),
                                  " vs ",
                                  paste0(round(barplot_df$mean[barplot_df$population == click$barplot_population & barplot_df$group == reactV$groups_names[-1]],1),"%")),
                   foldchange = round(2^(volcano_df$l2fc[volcano_df$population == click$barplot_population]),1),
                   l2fc = round(volcano_df$l2fc[volcano_df$population == click$barplot_population],1),
                   pvalue = scientific(volcano_df$pvalue[volcano_df$population == click$barplot_population],2),
                   distancefrom0 = round(volcano_df$distancefrom0[volcano_df$population == click$barplot_population],1))
  ...
  # Output Hieracheatmaps #############
    ...
    HHM_df <- reactive ({
      ...
      if(input$test_values_selector == "normCount"){
        HHM_df$l2fc = HHM_df$parentFreq_l2fc
        HHM_df$minuslog10pvalue = HHM_df$normCount_minuslog10pvalue
        HHM_df$distancefrom0 = HHM_df$normCount_distancefrom0
      }
    ...
    L1_Hieracheatmap <- reactive({
      if(is.null(HHM_df())| is.null(input$test_values_selector))
        return(NULL)
      if(is.null(input$HHM_stat_picker))
        return(NULL)
      HHM_df = HHM_df()
      stat = input$HHM_stat_picker
      if(stat == "refgroup_freq" & input$test_values_selector == "totalFreq"){ stat = "totalFreq_refgroup_mean"}
      if(stat == "refgroup_freq" & input$test_values_selector == "parentFreq"){ stat = "parentFreq_refgroup_mean"}
      if(stat == "refgroup_freq" & input$test_values_selector == "normCount"){ stat = "normCount_refgroup_mean"}
      if(stat == "group2_freq" & input$test_values_selector == "totalFreq"){ stat = "totalFreq_group2_mean"}
      if(stat == "group2_freq" & input$test_values_selector == "parentFreq"){ stat = "parentFreq_group2_mean"}
      if(stat == "group2_freq" & input$test_values_selector == "normCount"){ stat = "normCount_group2_mean"}
      HHM_df$stat = HHM_df[,stat]
      max = max(HHM_df$stat)
      min = min(HHM_df$stat)
      df = HHM_df[HHM_df$level == "1",]
      
      pop_df = data.frame(fullname = df$population)
      pop_df$sorting_pop = gsub(".*_","", pop_df$fullname)
      pop_df$sorting_prm = gsub("[+-].*","", pop_df$sorting_pop)
      pop_df$sorting_side = gsub(".*[-]","-",gsub(".*[+]","+", pop_df$sorting_pop))
      pop_df$sorting_side = factor(pop_df$sorting_side , levels = c("-","+","+hi","+lo", "-&lo"))
      if(input$sort_by_side_first == TRUE){
        pop_df = arrange(pop_df, sorting_side, sorting_prm)  
      }else{pop_df = arrange(pop_df, sorting_prm, sorting_side)}
      
      df$population = factor(df$population, levels = rev(pop_df$fullname))
  ...
  # Output dotplots from HHM click population ###############
    ...
    output$Dotplots <- renderPlot({
      ...
      subpop = "none"
      subpop_filter = rep(TRUE,dim(reactV$channel_values_matrix)[1])

      if(length(HHM_pop_vector) >= 2){
        filter_pop = HHM_pop_vector[-length(HHM_pop_vector)]
        subpop = paste(filter_pop,collapse = "_")
        subpop_filter = rowSums(reactV$L1_pops_belonging_matrix[,filter_pop, drop = FALSE]) == length(filter_pop)
      }
      submat = reactV$channel_values_matrix[subpop_filter,c(xvar,yvar)]
      subrowData = reactV$rowData[subpop_filter,]


      df = as.data.frame(cbind(subrowData[,c("group", "sample")], submat))
      df$aboveThresholdsum_xvar = reactV$aboveThresholdsum_matrix[subpop_filter,xvar]
      ...
      for(i in 1:length(unique(df$sample))) {
        ...
        if(reactV$thresholds[reactV$thresholds$parameter == xvar, "number_of_threshold"] == "1"){
          posEvent_count = sum(subdf$aboveThresholdsum_xvar == 1)
        ...
        if(reactV$thresholds[reactV$thresholds$parameter == xvar, "number_of_threshold"] == "2"){
          negEvent_count = sum(subdf$aboveThresholdsum_xvar == 0)
          negEvent_freq = negEvent_count / totalEvent_count
          
          posEvent_count = sum(subdf$aboveThresholdsum_xvar %in% c(1,2))
          posEvent_freq = posEvent_count / totalEvent_count
          
          hiposEvent_count = sum(subdf$aboveThresholdsum_xvar == 2)
          hiposEvent_freq = hiposEvent_count / totalEvent_count
          
          loposEvent_count = sum(subdf$aboveThresholdsum_xvar == 1)
          loposEvent_freq = loposEvent_count / totalEvent_count
          
          negnloEvent_count = sum(subdf$aboveThresholdsum_xvar %in% c(0,1))
          negnloEvent_freq = negnloEvent_count / totalEvent_count
        }
        ...
        if(is.na( reactV$thresholds[reactV$thresholds$parameter == xvar,"second_value"]) == FALSE){
              ...
              g = g + geom_vline(xintercept = reactV$thresholds[reactV$thresholds$parameter == xvar,"second_value"], size = 1, color = "red") +
                annotate(geom = "label", x = xmin, y = ymax, label = paste0(negEvent_count, " events\n", round(100*negEvent_freq,1),"%"),hjust =0, vjust = 1, label.size=NA,fill="white", alpha = 0.8)+
                annotate(geom = "label", x = xint, y = ymax, label = paste0(loposEvent_count, " events\n", round(100*loposEvent_freq,1),"%"),hjust =0.5, vjust = 1, label.size=NA,fill="white", alpha = 0.8)+
                annotate(geom = "label", x = xmax, y = ymax, label = paste0(hiposEvent_count, " events\n", round(100*hiposEvent_freq,1),"%"),hjust =1, vjust = 1, label.size=NA,fill="white", alpha = 0.8)+
                annotate(geom = "label", x = reactV$thresholds[reactV$thresholds$parameter == xvar,"value"], y = ymin, label = paste0(negnloEvent_count, " events\n", round(100*negnloEvent_freq,1),"%"),hjust =0.5, vjust = 0, label.size=NA,fill="white", alpha = 0.8)+
                annotate(geom = "label", x = reactV$thresholds[reactV$thresholds$parameter == xvar,"second_value"], y = ymin, label = paste0(posEvent_count, " events\n", round(100*posEvent_freq,1),"%"),hjust =0.5, vjust = 0, label.size=NA,fill="white", alpha = 0.8)
        }else{
           g = g +
            annotate(geom = "label", x = xmax, y = ymax, label = paste0(posEvent_count, " events\n", round(100*posEvent_freq,1),"%"),hjust =1, vjust = 1,label.size=NA,fill="white", alpha = 0.8)+
            annotate(geom = "label", x = xmin, y = ymax, label = paste0(negEvent_count, " events\n", round(100*negEvent_freq,1),"%"),hjust =0, vjust = 1,label.size=NA,fill="white", alpha = 0.8)
        }
      ...
      reactV$Dotplot_height = length(unique(reactV$rowData$sample))*200
